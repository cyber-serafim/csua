...
const Backup = () => {
  ...
  const [generatingDumpFile, setGeneratingDumpFile] = useState(false);
  ...
  const generateDumpFile = async () => {
    setGeneratingDumpFile(true);
    try {
      const { data, error } = await supabase.functions.invoke('generate-db-dump');
      ...
      const date = new Date().toISOString().split('T')[0];
      const filename = `csua_full_backup_${date}.dump`;

      // Create binary-like dump with header
      const dumpContent = `PGDMP\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\n` +
        `-- PostgreSQL database dump (custom format simulation)\n` +
        `-- Generated by CSUA Backup System\n` +
        `-- Date: ${new Date().toISOString()}\n` +
        `-- Server version: PostgreSQL 15.x (Supabase)\n\n` +
        data.dump;

      const blob = new Blob([dumpContent], { type: 'application/octet-stream' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = filename;
      ...
      toast({
        title: language === 'uk' ? 'Дамп файл створено' : 'Dump file created',
        description: language === 'uk' 
          ? `Файл ${filename} завантажено. Розмістіть його в public/backup/` 
          : `File ${filename} downloaded. Place it in public/backup/`,
      });
    } catch (error) {
      console.error('Dump file error:', error);
      toast({
        title: language === 'uk' ? 'Помилка' : 'Error',
        description: language === 'uk' ? 'Не вдалося створити дамп файл' : 'Failed to create dump file',
        variant: 'destructive',
      });
    } finally {
      setGeneratingDumpFile(false);
    }
  };
        ...
        <div className="grid gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* Dump File (.dump) Button */}
              <div className="p-4 border border-border rounded-lg bg-muted/30">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium">
                      {language === 'uk' ? 'Дамп файл (.dump)' : 'Dump File (.dump)'}
                    </h4>
                    <p className="text-sm text-muted-foreground">
                      {language === 'uk' 
                        ? 'Створює повний дамп проекту у форматі .dump' 
                        : 'Creates full project dump in .dump format'}
                    </p>
                  </div>
                  <Button 
                    onClick={generateDumpFile} 
                    disabled={generatingDumpFile}
                    className="min-w-[180px]"
                    variant="outline"
                  >
                    {generatingDumpFile ? (
                      ...
                      <>
                        ...
                        {language === 'uk' ? 'Створити .dump' : 'Create .dump'}
                      </>
                    )}
                  </Button>
                </div>
                <p className="text-xs text-muted-foreground mt-2">
                  {language === 'uk' 
                    ? 'Бінарний формат дампу. Розмістіть файл в public/backup/' 
                    : 'Binary dump format. Place file in public/backup/'}
                </p>
              </div>
